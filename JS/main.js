// === CONFIGURACIÓN Y CONTENIDO INTEGRADO ===
...
// === CONTENIDO TEÓRICO (integrado para evitar problemas de ámbito) ===
const LESSON_TITLES = [
...
const LESSON_TEXTS = {
  ...
  // Por brevedad, aquí van los textos 3 a 29 (puedes copiarlos del archivo anterior)
...
function getLessonTitle(day) {
  return LESSON_TITLES[day - 1] || `Lección ${day}`;
}

function getLessonText(day) {
  return LESSON_TEXTS[day] || `<p>Contenido no disponible para el día ${day}.</p>`;
}
...
function renderSidebar() {
  ...
  const modules = [
    { id: 1, title: "Módulo 1: Planteamiento", days: 7, locked: false },
      ...
      <div class="module" data-module="${module.id}">
        ...
        <div class="module-content" id="module${module.id}" style="${module.id === 1 ? 'display: block;' : 'display: none;'}">
          ${Array.from({length: module.days}, (_, i) => {
            ...
            return `<div class="${itemClass}" data-day="${day}" onclick="showLessonIfUnlocked(${day})">Día ${day}: ${getLessonTitle(day)}</div>`;
          }).join('')}
        </div>
      </div>
    `;
...
function toggleModuleIfUnlocked(moduleId) {
  ...
  if (moduleId === 2 && !isModule1Complete()) {
    alert("⚠️ Debes completar al menos 5 lecciones del Módulo 1 para acceder.");
    return;
  }
  if (moduleId === 3 && !isModule2Complete()) {
    alert("⚠️ Debes completar al menos 5 lecciones del Módulo 2 para acceder.");
    return;
  }
  if (moduleId === 4 && !isModule3Complete()) {
    alert("⚠️ Debes completar al menos 6 lecciones del Módulo 3 para acceder.");
    return;
  }
...
function activateSidebarItem(day) {
  ...
  if (currentItem) {
    ...
    // Asegurar que el módulo esté visible
    const moduleMap = {
...
function showLessonIfUnlocked(day) {
  if (isDayUnlocked(day)) {
    ...
    alert(`⚠️ Debes completar al menos ${module === 1 ? 5 : module === 2 ? 5 : 6} lecciones del Módulo ${module} para acceder.`);
  }
}
...
function showLesson(day) {
  ...
  container.innerHTML = `
    <div class="lesson-content active">
      <div class="lesson-header"><h2>Día ${day}: ${getLessonTitle(day)}</h2></div>
